...
import { getUser, updateUserBank } from "../../api/user";
...
const SupplierBank = () => {
  ...
  useEffect(() => {
    ...
    const loadUserData = async () => {
      ...
      if (user && user.supplierWallet) {
        setBankAccount(user.supplierWallet.bankAccount);
        setBankNumber(user.supplierWallet.bankNumber);
        const bank = bankList.find(
          (bank) => bank.name === user.supplierWallet.bankName
        );
        if (bank) {
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      // Call the updateSupplierBank API function
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-4">
          <Form className="bg-white p-4 rounded-lg" onFinish={handleFormSubmit}>
            ...
            >
              <Select
                value={selectedBank.id || ""} // Preselect the bank if already chosen
